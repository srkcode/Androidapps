package example.srk.currencyapp.utils;
    ...
    public static JSONObject requestJSONObject(String serviceURL) {
        ...
        try {
            ...
            if(statusCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
                example.srk.currencyapp.utils.LogUtils.log(TAG, "Unauthorized access!");
            } else if(statusCode == HttpURLConnection.HTTP_NOT_FOUND) {
                example.srk.currencyapp.utils.LogUtils.log(TAG, "404 page not found");
            } else if(statusCode != HttpURLConnection.HTTP_OK) {
                example.srk.currencyapp.utils.LogUtils.log(TAG, "URL Response error");
            }

            InputStream in = new BufferedInputStream(urlConnection.getInputStream());
            return new JSONObject(convertInputStreamToString(in));
        } catch(MalformedURLException e) {
            example.srk.currencyapp.utils.LogUtils.log(TAG, e.getMessage());
